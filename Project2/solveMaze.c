...
void enqueue ( Queue* queue, graphNode_t graphNode ) {
...
int main ( int argc, char* argv[] ) {
    ...
    size_t numNode = adjMatrixToList( argv[1], &adjacencyList);

    // USE A QUEUE TO PERFORM BFS
    Queue queue = { .front=NULL, .back=NULL };

    // An array that keeps track of who is the parent node of each graph node we visit
    graphNode_t* parents = calloc( numNode, sizeof(graphNode_t) );
    ...
    free (parents);
    freeAdjList ( numNode, adjacencyList );
