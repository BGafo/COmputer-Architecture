...
void accessData (
    ...
    for ( int bit= 7; 0<=bit; bit-- ) {
        size_t bit_val = ((1<<4)-1) & addr>>bit*4; // shift and mask
        if(bit==1){
            set_index = bit_val;
        }
    }
    mem_addr_t current_tag = addr/256;

    // hash function
    int hashCode = set_index % 16;

    // cache hit
    if(cache[hashCode].valid==1 && cache[hashCode].tag==current_tag){
        (*hit_count)++;
            return;
    }

    
     // If cache set_index is taken, evict the item and store new cache
    if ( cache[hashCode].valid==1 && cache[hashCode].tag != current_tag ) {
        cache[hashCode].valid = 1;
        cache[hashCode].tag = current_tag;
        (*eviction_count)++;
        (*miss_count)++;
    }
    // If cache set line already in use as indicated by the valid variable, then evict the existing cache set line
    if (cache[hashCode].valid==0 ) {
        cache[hashCode].valid = 1;
        cache[hashCode].tag = current_tag;
        (*miss_count)++;
    }
