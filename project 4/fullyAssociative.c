...
void accessData (
    ...
    // record cache miss
        ...
        cache_line_t* newCacheInput = malloc(sizeof(cache_line_t));;
        newCacheInput->tag = tag;
        newCacheInput->next_cache_line = NULL;
        if(cache->back == NULL){
            cache->back = newCacheInput;
            cache->front = newCacheInput;
        }       else {
                    cache->back->next_cache_line = newCacheInput;
                    cache->back = newCacheInput;
                }
        cache->occupancy++;
        return;
