...
void accessData (
    ...
    set_index = (addr>>b) & 3;
    mem_addr_t current_tag = (addr>>(b+s));

    // Find the correct cache set
    int hashCode = set_index % 4;


    size_t least_recent_use=cache[hashCode][0].recent_use;
        for(size_t j=0; j<E; j++){
            // Cache hit
            if(cache[hashCode][j].valid==1 && cache[hashCode][j].tag==current_tag ){
                cache[hashCode][j].recent_use = program_counter;
                (*hit_count)++;
            return;
            }
        } 
        ...
        for(size_t k=0; k<E; k++){
            ...
            if( cache[hashCode][k].recent_use < recent_use_holder){
                recent_use_holder = cache[hashCode][k].recent_use;
                least_recent_use = k;              
            }   
        } 
        // If cache set line already in use, then record an eviction
        if (!cache[hashCode][least_recent_use].valid) {
            cache[hashCode][least_recent_use].valid = 1;
        } else {
            (*eviction_count)++;
        }
        cache[hashCode][least_recent_use].tag = current_tag;
        cache[hashCode][least_recent_use].recent_use = program_counter; 
        return;   
}
